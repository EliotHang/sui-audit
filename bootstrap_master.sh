...
usage() {
    cat << EOF
用法: $0 [--update] [install_master.sh 参数]
...
main() {
    ...
    if [[ "$UPDATE_MODE" -eq 1 ]]; then
        log_info "更新 master: ${INSTALL_DIR}"
        update_master "$package_dir"
    else
        log_info "安装 master 到: ${INSTALL_DIR}"
        SUI_AUDIT_MASTER_DIR="$INSTALL_DIR" bash "$package_dir/install_master.sh" "${INSTALL_ARGS[@]}"
    fi
